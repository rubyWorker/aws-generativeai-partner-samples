...
if (!awsAccessKeyId || !awsSecretAccessKey) {
    console.error("AWS credentials not provided. Please set AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY environment variables or use --aws-access-key-id and --aws-secret-access-key arguments");
    process.exit(1);
}
...
server.tool("send-email", "Send an email using AWS SES", {
    ...
    html: z
        .string()
        .optional()
        .describe("HTML email content. When provided, the plain text argument MUST be provided as well."),
    cc: z
    ...
    if (typeof replyToEmailAddresses !== "string" &&
    ...
    try {
        ...
        throw new Error(`Email failed to send: ${error instanceof Error ? error.message : String(error)}`);
    }
});
async function main() {
